...
#if 1
int write_image(QRcode *qrcode, unsigned char* buf)
{
	...
	row = (unsigned char *)malloc((realwidth));

	/* top margin */
	memset(row, 0xff, realwidth );
	for(y=0; y< margin * size; y++) {
		memcpy(&buf[y * realwidth], row, realwidth);
		height++;
//		png_write_row(png_ptr, row);
	...
	for(y=0; y<qrcode->width; y++) {
		...
		for(yy=0; yy<size; yy++) {
			memcpy(&buf[height * realwidth], row, realwidth);
			height++;
			//png_write_row(png_ptr, row);
		}
	}
...
void qrencode_free(QRcode* code)
{
    QRcode_free(code);
}
