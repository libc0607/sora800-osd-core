...
MAP_OLD_FILE := ${SOFT_WORKDIR}/platform/compilation/${CT_ASIC}_map_cfg
MAP_FILE := ${SOFT_WORKDIR}/platform/compilation/${CT_ASIC}_new_map_cfg
MAP_CFG_GEN_FILE := ${SOFT_WORKDIR}/platform/compilation/map_cfg_gen.pl
...
targetgen: force
	...
	@${ECHO} "SPIFFS_FLASH_BASE:=    $(SPIFFS_FLASH_BASE)"      >> $(TARGET_FILE)
	@${ECHO} "SPIFFS_FLASH_SIZE:=    $(SPIFFS_FLASH_SIZE)"      >> $(TARGET_FILE)
...
ifeq ($(strip $(AM_SUBPROJ_SUPPORT)), TRUE)
	...
	perl ${MAP_CFG_GEN_FILE} -l ${MAP_OLD_FILE} -o ${MAP_FILE} -i ${SPIFFS_FLASH_BASE} -f ${FLSH_MODEL}
	${MAKE} targetgen
