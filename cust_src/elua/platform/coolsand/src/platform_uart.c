...
/*+\NEW\zhutianhua\2018.2.6 10:31\新增清空uart buf的clear接口*/
...
static void debugPortWrite(const u8 *buff, u16 len)
{
    ...
    for(i = 0; i < len; i++)
    {
        ...
        /*begin\NEW\zhutianhua\2017.10.9 13:34\修正host口输出的trace中包含%时，输出打印会丢失字符的问题*/
        else if(data == '%')
        {
        ...
        /*end\NEW\zhutianhua\2017.10.9 13:34\修正host口输出的trace中包含%时，输出打印会丢失字符的问题*/
        else
        {
...
/*+\NEW\zhutianhua\2018.2.6 10:27\新增清空uart buf的clear接口*/
u32 platform_uart_clear( unsigned id, u32 bufType)
{
    u32 ret = PLATFORM_ERR;
    
    if((id < NUM_UART) && (bufType<PLATFORM_UART_BUF_NONE)) //非特殊端口才可以关闭
    {
