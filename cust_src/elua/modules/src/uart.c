...
/*+\NEW\zhutianhua\2018.2.6 10:25\新增清空uart buf的clear接口*/
static int uart_clear( lua_State* L )
{
    u32 id = luaL_checkinteger(L,1);
    u32 bufType = luaL_checkinteger(L,2);
    u32 res;

    res = platform_uart_clear(id, bufType);
...
/*-\NEW\liweiqiang\2013.4.20\增加uart.close接口 */
  /*+\NEW\zhutianhua\2018.2.6 10:24\新增清空uart buf的clear接口*/
  { LSTRKEY( "clear" ),  LFUNCVAL( uart_clear ) },
  /*-\NEW\zhutianhua\2018.2.6 10:24\新增清空uart buf的clear接口*/
...
LUALIB_API int luaopen_uart( lua_State *L )
{
  ...
  /*+\NEW\zhutianhua\2018.2.6 10:23\新增清空uart buf的clear接口*/
  MOD_REG_NUMBER( L, "SEND_BUF", PLATFORM_UART_SEND_BUF );
  MOD_REG_NUMBER( L, "RECV_BUF", PLATFORM_UART_RECV_BUF );
  /*-\NEW\zhutianhua\2018.2.6 10:23\新增清空uart buf的clear接口*/
